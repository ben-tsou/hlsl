...
from LSL_utils import length_filtering_h2_3
...
def main(main_anneal='logistic',
         ...
         main_torch_manual_seed=627, main_np_random_seed=627, main_env_seed=6359):
    ...
    model_file = ('saved_models_h3/model' +
    ...
    z_recon_stats_file = ('z_recon_stats_h3/' + 'full_' + current_time + '.txt')
    z_lengths_file = ('z_lengths_h3/' + 'full_' + current_time + '.txt')

    z_trajectory_actions_file = ('z_trajectory_actions_h3/' + 'full_' + current_time + '.txt')
    z_trajectory_env_states_file = ('z_trajectory_env_states_h3/' + 'full_' + current_time + '.txt')
    ...
    for name, param in named_params:
        ...
        if "h1" in name:
            ...
            if "encode" in name:
                h3_encoder_params += [param]
            elif "decode" in name:
                h3_decoder_params += [param]
            else:
                h3_decoder_params += [param]
    ...
    h_param_groups = [{'params': h1_encoder_params, 'g_name': 'h1_encoder_params'},
                      ...
                      {'params': h3_encoder_params, 'g_name': 'h3_encoder_params'},
                      {'params': h3_decoder_params, 'g_name': 'h3_decoder_params'},
                      ]

    new_optimizer = optim.AdamW(h_param_groups, lr=Adam_learning_rate, weight_decay=0.0)

    set_param_grad_status(new_optimizer, ['h3_encoder_params', 'h3_decoder_params'],
                                         ['h2_encoder_params', 'h2_decoder_params',
    ...
    for compress_iteration in range(num_compression_iterations):
        ...
        log_dir = os.path.join('LSL_new_runs_h3', current_time + '_' +
                               anneal + '_num_epochs_' + str(num_epochs) +
                               "comp_iter_" + str(compress_iteration) + '_' + str(torch_manual_seed))

        print(log_dir)

        run_VAE_statsfile = ('LSL_run_VAE_stats_h3/' + 'full' +
                             anneal + '_num_epochs_' + str(num_epochs) +
        ...
        if compress_iteration == 0 and bootstrap == "no":
            ...
            pos_to_sample = find_good_region(new_model, z_grid, z_seq_len, primitive_trajectory_len,
                                         ...
                                         hierarchy_level="h3", lengths_metric=lengths_metric,
        ...
        if compress_iteration == 0 and bootstrap == "no":
            ...
            _, _, _, trajectory_states_argmax, argmax_lengths = \
                new_model.argmax_or_repulsion_from_region_h(pos_to_sample, argmax=True, env=env,
                                                            hierarchy_level="h3", random_traj=True)
        else:

            trajectory_actions_argmax, trajectory_actions_one_hot_argmax, \
            _, _, _, trajectory_states_argmax, argmax_lengths = \
                new_model.argmax_or_repulsion_from_region_h(pos_to_sample, argmax=True, env=env,
                                                            hierarchy_level="h3", bootstrap="no",
            ...
            if bootstrap != "no":
                ...
                _, _, _, trajectory_states_argmax2, argmax_lengths2 = \
                    new_model.argmax_or_repulsion_from_region_h(pos_to_sample, z_grid, argmax=True, env=env,
                                                                hierarchy_level="h3", bootstrap=bootstrap,
        ...
        if compress_iteration > 0 or bootstrap != "no":
            ...
            first_trajectory_actions_repulsion, first_trajectory_actions_one_hot_repulsion, \
                ...
                new_model.argmax_or_repulsion_from_region_h(pos_to_sample,
                                                            z_grid,
            ...
            third_trajectory_actions_repulsion, third_trajectory_actions_one_hot_repulsion, \
                ...
                new_model.argmax_or_repulsion_from_region_h(pos_to_sample,
                                                            z_grid, argmax=False, env=env, hierarchy_level="h3")
        ...
        if compress_iteration < main_num_logistic_compression_iterations and \
                (bootstrap != "no" or compress_iteration > 0):
            ...
            lengths = torch.cat([argmax_lengths, first_repulsion_lengths, second_repulsion_lengths, third_repulsion_lengths])
            ...
            if bootstrap != "no":
                ...
                trajectory_actions = trajectory_actions2
                trajectory_actions_one_hot = trajectory_actions_one_hot2

                trajectory_env_states = trajectory_env_states2
                lengths = lengths2

            max_length = new_model.h3_top_seq_len * new_model.h3_bottom_seq_len * new_model.h2_bottom_seq_len

            if bootstrap == "no":

                lengths, trajectory_actions, trajectory_actions_one_hot, trajectory_env_states, _ = \
                    length_filtering_logistic(lengths, argmax_lengths, trajectory_actions,
                                              trajectory_actions_one_hot,
                                              trajectory_env_states, max_length, 50, global_cutoff_length=None)

            else:

                lengths, trajectory_actions, trajectory_actions_one_hot, trajectory_env_states = \
                    length_filtering_h2_3(lengths, argmax_lengths2, trajectory_actions, trajectory_actions_one_hot,
                                              trajectory_env_states, max_length, 50)
            ...
            if bootstrap != "no":
                ...
                trajectory_actions = trajectory_actions_argmax2
                trajectory_actions_one_hot = trajectory_actions_one_hot_argmax2
                trajectory_env_states = trajectory_states_argmax2
                lengths = argmax_lengths2

            lengths_np = lengths.clone().numpy()
            length_percentile_cutoff = 50
            cutoff_length = np.percentile(lengths_np, length_percentile_cutoff)
        ...
        run_VAE(new_model, new_optimizer, trajectory_env_states, trajectory_actions, trajectory_actions_one_hot,
                ...
                floor=floor, log_interval=1, log_epoch_interval=1,
                f=f, writer=writer, hierarchy_level="h3", lengths=lengths)

        iteration_model_file = ('saved_models_full/model_h3' +
        ...
        if compress_iteration == num_compression_iterations - 1:
            ...
            final_model_file = ('saved_models_full/model_h3_final' +
                                anneal + 'comp_iter' + str(compress_iteration) + '_' +
        ...
        trajectory_diagnostics(model=new_model, z_space_folder='z_space_trajectories_h3', env=env,
                               ...
                               z_recon_stats_file=z_recon_stats_file, hierarchy_level="h3",
